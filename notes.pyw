...
class PersistentNotepad:
    def __init__(self):
        ...
        self.auto_save = True
        self.always_on_top = True  # Nova variável para controlar "sempre no topo"
        ...
        # Configurar para manter na frente (opcional)
        # self.root.attributes('-topmost', True)  # Descomente se quiser iniciar sempre no topo
    ...
    def load_settings(self):
        ...
        default_settings = {
            ...
            "always_on_top": True  # Nova configuração
        }
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            "always_on_top": self.always_on_top  # Salva o estado
        }
    ...
    # NOVA FUNÇÃO: Alternar "Sempre no Topo"
    def toggle_always_on_top(self):
        """Alterna entre manter a janela sempre visível ou não"""
        self.always_on_top = not self.always_on_top
        self.root.attributes('-topmost', self.always_on_top)
        
        # Atualizar o menu para mostrar o estado atual
        menu = self.root.nametowidget(self.root["menu"])
        file_menu = menu.children["!menu"]
        
        # Encontrar e atualizar o item do menu
        menu_items = file_menu.index("end")
        for i in range(menu_items + 1):
            try:
                label = file_menu.entrycget(i, "label")
                if label == "Sempre no Topo":
                    # Adicionar ou remover a marcação de seleção
                    if self.always_on_top:
                        file_menu.entryconfig(i, label="✓ Sempre no Topo")
                    else:
                        file_menu.entryconfig(i, label="Sempre no Topo")
                    break
            except:
    ...
    def find_text(self):
        ...
        tk.Label(find_window, text="Buscar:").pack(pady=5)
    ...
    def change_font(self):
        ...
        tk.Label(font_window, text="Família da Fonte:").pack(pady=5)
        
        font_families = list(font.families())
        font_families.sort()
        
        font_var = tk.StringVar(value=self.current_font[0])
        font_listbox = tk.Listbox(font_window, height=5)
        ...
        tk.Label(font_window, text="Tamanho:").pack(pady=5)
        size_var = tk.StringVar(value=str(self.font_size))
        size_spinbox = tk.Spinbox(font_window, from_=8, to=72, textvariable=size_var)
    ...
    def change_text_color(self):
        ...
        if color[1]:
            ...
            self.text.config(fg=self.text_color)
            self.save_settings()
